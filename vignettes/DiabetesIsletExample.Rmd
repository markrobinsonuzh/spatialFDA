...
metricRes <- calcMetricPerFov(spe, c("alpha", "Tc"),
    ...
    rSeq = seq(0, 50, length.out = 50), by = c("patient_stage", "patient_id", "image_number"),
    ncores = 1
)
...
```{r plotLfunction}
p <- plotMetricPerFov(metricRes, correction = "border", x = "r", imageId = "image_number", ID = "ID")
print(p)
```
...
```{r plotGfunction}
p <- plotMetricPerFov(metricRes, correction = "rs", x = "r", imageId = "image_number", ID = "ID")
print(p)
```
...
plotLs <- lapply(colnames(designmat), plotMdl, mdl = mdl, shift = mdl$coefficients[["(Intercept)"]])
...
```{r fPCA}
# calculate the fPCA
pca <- functionalPCA(dat = dat, r = metricRes$r |> unique(), knots = 30, pve = 0.99)
