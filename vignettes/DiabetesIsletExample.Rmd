...
abstract: >
  ...
  functional principal component analysis adn compare them with 
...
p <- plotMetricPerFov(metricRes, correction = "border", x = "r",
...
p <- plotMetricPerFov(metricRes, correction = "rs", x = "r",
                      imageId = "image_number", ID = "ID")
print(p)
```

In the nearest-neighbor distance function we see a strong difference between long duration T1D, onset T1D and non-diabetic controls in terms of spacing of $\alpha$ and cytotoxic T cells. 
...
# filter out all rows that have a constant zero part - all r<5
...
We note that there is no difference in the $G$ function between non-diabetic and long duration T1D samples but a strong difference between non-diabetic and onset T1D. This difference is significant after $\sim 30 µm$ and increases at longer distances. Therefore, we don't only see that a spatial difference colocalisation of $\alpha$ and cytotoxic T cells is statistically significant but also at which spatial scale this difference occurs.

One open problem is the implementation of confidence bands that reflect the non-independently and non-identically distributed residuals. To visualise how much of a problem this is, we can plot the contours of the correlation/covariance and look at some model diagnostics. We can model the confidence using a gaussian location-scale model if the errors are heteroskedastic [@wood2016Smoothing]. If does not result in a good model fit, we have to implement bootstrapping of confidence bands.

```{r contour, warning = FALSE}
resid(mdl) |> cov() |> filled.contour()
# TODO atm some of the plots fail - potentially due to NA in mdl output
try(refund::pffr.check(mdl))
```

In these model diagnostics we note that there is still significant variability in the residuals that is not considered by the model. In this case we should resort to bootstrapping to obtain better confidence bands for the inference.
